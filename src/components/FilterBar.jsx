export default function FilterBar({
  ...
  return (
    <div className="bg-parchment text-wood p-6 rounded-lg border-2 border-gold space-y-4">
      <h2 className="text-2xl font-medieval font-bold text-gold-dark">Search & Filter</h2>

      <div>
        <input
          type="text"
          placeholder="Search by name or VRC player name..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full px-4 py-2 border-2 border-gold-dark rounded bg-parchment-dark text-wood placeholder-wood-light focus:outline-none focus:border-gold"
        />
      </div>
...
function FilterSelect({ label, value, onChange, options }) {
  return (
    <div>
      <label className="block text-xs text-gold-dark uppercase tracking-wide font-medieval mb-2">
        {label}
      </label>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full px-3 py-2 border-2 border-gold-dark rounded bg-parchment-dark text-wood focus:outline-none focus:border-gold font-medieval"
      >
        {options.map(opt => (
